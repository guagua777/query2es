...
public class Transfer {
    ...
    public static class SortAndScroll{
...
//        pageSizeCheck(filter);
    ...
    public SearchSourceBuilder transferWithOutCheckValue(String filter, SearchType searchType) throws IOException {
        return transferWithOutCheckValue(filter, -1, searchType);
    }

    public SearchSourceBuilder transferWithOutCheckValue(String filter, int timeout, SearchType searchType) throws IOException {
        //1.去掉空格
        String f1 = filter.replaceAll(" ", "");
        filter = f1;
        if (filter.length() > 2000){
...
//        pageSizeCheck(filter);
        ...
        TokenReader tokenReader = new TokenReader(filter);
        List<Token> tokens = tokenReader.getTokensWithOutCheckValue();
        AstNode astNode = transferAst(tokens);
        SearchSourceBuilder searchSourceBuilder = transferES(astNode, timeout, searchType);
        return searchSourceBuilder;
    }
...
//        String str = "a=b&c=d&page=-1&size=20";
        ...
        new Transfer().printFilter(str);
        SearchSourceBuilder transfer = new Transfer().transfer(str, 2000, SearchType.QUERY);
